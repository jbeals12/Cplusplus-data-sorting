...
private:
	...
	function<int(KEY, int)> hash;	//The hash function.  You would use it as if it was a private method declared like so:
	int arraySize;					//int hash(KEY key, int arraySize) { /*does math*/ return someInt; }
};

template <typename KEY, typename VALUE>
arrayHashTable<KEY, VALUE>::arrayHashTable(function<int(KEY, int)> hashFunction, int arraySize)
{
	//This just makes sure the object remebers the hash function pointer that was passed into this constructor.
	hash = hashFunction;
...
void arrayHashTable<KEY, VALUE>::add(const KEY& key, const VALUE& value)
{
	...
	int startingBucket = possibleBucket;

	while (statusArray[possibleBucket] == 1)
	{
		possibleBucket++;

		if (possibleBucket == startingBucket)
		{
			throw Error();
		}

		if (possibleBucket == arraySize - 1)
		{
			possibleBucket = 0;
		}
	}
...
bool arrayHashTable<KEY, VALUE>::exists(const KEY& key) const
{
	...
	for (int i = possibleBucket; i < arraySize; i++)
	{
		if (statusArray[i] == 0)
		{
			return false;
		...
		else if (keyArray[possibleBucket] == keyArray[i] && statusArray[i] == 1)
		{
			return true;
		}
	}
...
VALUE arrayHashTable<KEY, VALUE>::item(const KEY& key)
{
	...
	//Remember that if you hit the end of the array, you need to wrap around and 
	//start back again until you have searched all possibilities.
	//If you don't find anything, throw an Error object.
